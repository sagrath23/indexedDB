...
// Compare two keys.
Object.defineProperty(IDBFactory, 'cmp', {
    value: (a, b) => {
        if (a < b) return -1;
    ...
    constructor(request, range = undefined, direction = 'next', withValue = false) {
        ...
        function _continue(targetKey = undefined) {
            ...
            if (targetKey !== undefined)
                while (primaryKey !== undefined && !keyInRange(key, targetKey)) progress();
...
class IDBKeyRange {
    ...
    constructor(lower, upper, lowerOpen = false, upperOpen = false) {
        ...
        function includes(key) {
            ...
            if (upper !== undefined) {
                if (upperOpen) { if (key >= upper) return false; } else { if (key > upper) return false; }
            }
            if (lower !== undefined) {
                if (lowerOpen) { if (key <= lower) return false; } else { if (key < lower) return false; }
            }
            return true;
        }
    }
